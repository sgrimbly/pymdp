...
class Agent(Module):
    ...
    #
    # `static=True` is safe here: ``Policies`` (in pymdp.control) is now
    # backed by a hashable nested-tuple ``_policy_tup`` field rather
    # than a ``jnp.ndarray``, so equinox's static-partition equality
    # is by-value rather than buffer-identity. This closes upstream
    # issue #346 (the recompile cliff documented at PR #345's merge)
    # while preserving the vmap/scan-over-Agent property #345 was
    # introduced for. See ``pymdp/control.py:Policies`` for the
    # tuple-backed implementation and the downstream rationale at
    # ``docs/architecture/pymdp_static_array_fork_fix.md`` in
    # Hierarchical_AttentionAgent.
